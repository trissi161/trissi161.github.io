...
# WICHTIG: Prüfe, ob diese URL aus deinem Apps Script noch aktuell ist!
...
with tab_admin:
    ...
    if pw == "2504":
        admin_sub1, admin_sub2, admin_sub3 = st.tabs(["📊 Team-Übersicht", "⚠️ Verwarnungen", "🛠 Bearbeitungs-Modus"])
        
        with admin_sub1:
            st.subheader("Aktueller Team-Status")
            st.dataframe(style_team_table(df_personal), use_container_width=True, height=400)
            st.divider()
                ...
                if st.form_submit_button("Verwarnung rechtskräftig machen"):
                    if v_grund:
                        ...
                        payload = {
                            "sheet": "P", 
                            "action": "update_all", 
                            "headers": df_p_new.columns.tolist(), 
                            "rows": df_p_new.values.tolist()
                        }
                        requests.post(WEBHOOK_URL, data=json.dumps(payload))
                        st.success(f"⚠️ Verwarnung für {v_target} wurde registriert und der Zähler erhöht!")
                        st.rerun()
        ...
        with admin_sub3:
            ...
            # (Personal-Editor bleibt gleich wie in t-ffrp (3).py)
            ...
            if st.button("Personal-Daten speichern"):
                ...
                st.rerun()
            # (Berichte-Editor bleibt gleich wie in t-ffrp (3).py)
            st.divider()
